...
public:
    ...
    shared_state_(std::make_shared<SharedState>(*shared_state))
    {}

    Flyweight(const Flyweight& other) :
    shared_state_(std::make_shared<SharedState>(*other.shared_state_))
    {}

    ~Flyweight() = default;

    std::shared_ptr<SharedState> shared_state() const
    {
...
public:
    ...
    FlyweightFactory(std::initializer_list<SharedState> share_states)
    {
       for (const SharedState& ss : share_states)
       {
          this->flyweights_.insert(std::make_pair<std::string, Flyweight>(this->GetKey(ss),
                                                                          Flyweight(std::make_shared<SharedState>(ss))));
       }
    }
    ...
    Flyweight GetFlyweight(const SharedState &shared_state)
    {
       ...
       if (this->flyweights_.find(key) == this->flyweights_.end()) {
          ...
          this->flyweights_.insert(std::make_pair(key, Flyweight(std::make_shared<SharedState>(shared_state))));
...
int main()
{
   std::unique_ptr<FlyweightFactory> factory = std::make_unique<FlyweightFactory>(std::initializer_list<SharedState>{{"Chevrolet", "Camaro2018", "pink"}, {"Mercedes Benz", "C300", "black"}, {"Mercedes Benz", "C500", "red"}, {"BMW", "M5", "red"}, {"BMW", "X6", "white"}});
